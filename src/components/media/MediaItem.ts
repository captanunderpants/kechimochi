...
export class MediaItem extends Component<MediaItemState> {
    ...
    render() {
        ...
        const content = imgSrc 
            ? html`<img src="${imgSrc}" style="width: 100%; height: 100%; object-fit: cover; display: block;${media.nsfw ? ' filter: blur(20px); transition: filter 0.3s;' : ''}" alt="${media.title}" />`
            : html`
        ...
        if (media.nsfw && imgSrc) {
            ...
            this.container.addEventListener('mouseleave', () => {
                const img = this.container.querySelector('img');
                if (img) img.style.filter = 'blur(20px)';
            });
        }
    }
}
