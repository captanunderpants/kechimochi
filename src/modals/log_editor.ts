    ...
    return new Promise(async (resolve) => {
        ...
        overlay.innerHTML = `
            <div class="modal-content" style="max-width: 90vw; width: 900px; max-height: 85vh; display: flex; flex-direction: column;">
                ...
                <div style="display: grid; grid-template-columns: minmax(0, 2fr) 120px 110px 100px 60px; gap: 0.5rem; padding: 0.5rem 0.25rem; font-size: 0.75rem; font-weight: 600; color: var(--text-secondary); border-bottom: 1px solid var(--border-color);">
                    <span>Title</span>
                    <span>Duration</span>
        ...
        function renderRows(filter: string) {
            ...
            for (const log of filtered) {
                ...
                row.style.cssText = 'display: grid; grid-template-columns: minmax(0, 2fr) 120px 110px 100px 60px; gap: 0.5rem; padding: 0.4rem 0.25rem; align-items: center; border-bottom: 1px solid var(--border-color); font-size: 0.85rem;';
            ...
            listEl.querySelectorAll('.log-duration-input').forEach(input => {
                ...
                const save = async () => {
                    ...
                    try {
                        await updateLog(id, parsed, log.characters_read);
            ...
            listEl.querySelectorAll('.log-chars-input').forEach(input => {
                ...
                const save = async () => {
                    ...
                    try {
                        await updateLog(id, log.duration_minutes, parsed);
        ...
        overlay.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && document.activeElement === overlay) { cleanup(); resolve(dirty); }
        });
    });
}
