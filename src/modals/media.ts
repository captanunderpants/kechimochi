...
import { CONTENT_TYPES, getMediaTypeForContentType } from './activity';
...
export async function showAddMediaModal(): Promise<{title: string, type: string, contentType: string} | null> {
    return new Promise((resolve) => {
        ...
        const contentTypeSelect = overlay.querySelector('#add-media-content-type') as HTMLSelectElement;
        ...
        overlay.querySelector('#add-media-confirm')!.addEventListener('click', () => { 
            ...
            const mediaType = getMediaTypeForContentType(contentType);
            cleanup(); 
            resolve({ title, type: mediaType, contentType }); 
        });
        titleInput.focus();
    });
}
